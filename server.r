...
server = function(input, output)
{
    ...
    data1 = eventReactive(input$goButton, {
        ...
        if (input$typeInput == "hashtag") 
        {
            ...
            df.tweets = searchThis(search_string = input$hashtagInput,
                                     n = input$numberInput)
            
        } 
        
        #Cleans the tweet
        df.tweets = cleanTweets(df.tweets)
        #Get sentiments
    ...
    data2 = eventReactive(input$goButton, {
        ...
        {
            tweetOutput = userTL(user.name = input$usernameInput,n = input$numberInput)
        }
